...
describe('Node.isDefaultNamespace()', () => {
  ...
  it('converts the namespace before resolving the receiver context', () => {
    const html = document.createElement('div');
    const svg = document.createElementNS(SVG_NAMESPACE, 'svg');
    const text = document.createTextNode('text');
    ...
    const result = text.isDefaultNamespace({
      toString() {
        conversions++;
        svg.appendChild(text);
  ...
  it('walks deeply nested namespace contexts without recursion', () => {
    let current = document.createElement('div');
    for (let index = 0; index < 10_000; index++) {
      const next = document.createElementNS('urn:own', 'p:item');
      current.appendChild(next);
      current = next;
    }

    expect(current.isDefaultNamespace(HTML_NAMESPACE)).toBe(true);
  }, 20_000);
