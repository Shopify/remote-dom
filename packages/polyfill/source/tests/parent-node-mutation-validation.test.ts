...
describe('ParentNode insertion validation', () => {
  ...
  it('preserves self-insertion lifecycle and observer effects', () => {
    const calls: string[] = [];
    class IdentityInsertionElement extends HTMLElement {
      connectedCallback() {
    ...
    customElements.define(
      'identity-insertion-element',
      IdentityInsertionElement,
    );
    const parent = document.createElement('div');
    const child = document.createElement('identity-insertion-element');
    parent.appendChild(child);
    ...
    expect(calls).toEqual(['disconnected', 'connected']);
    expect(
...
describe('ParentNode replacement validation', () => {
  ...
  it('preserves final position when replacing a child with itself', () => {
    const parent = document.createElement('div');
    const first = document.createElement('span');
    const child = document.createElement('em');
    const last = document.createElement('strong');
  ...
  it('preserves self-replacement lifecycle and observer effects', () => {
    const calls: string[] = [];
    class IdentityReplacementElement extends HTMLElement {
      connectedCallback() {
        calls.push('connected');
      }
    ...
    customElements.define(
      'identity-replacement-element',
      IdentityReplacementElement,
    );
    const parent = document.createElement('div');
    const child = document.createElement('identity-replacement-element');
    parent.appendChild(child);
    document.body.appendChild(parent);
    calls.length = 0;
    const observer = new MutationObserver(() => {});
