...
const PRE_DISPATCH_STOP_FORMS = [
  ['stopPropagation()', (event: Event) => event.stopPropagation()],
  [
    'stopImmediatePropagation()',
    (event: Event) => event.stopImmediatePropagation(),
  ],
  ['cancelBubble = true', (event: Event) => (event.cancelBubble = true)],
] as const;
...
describe('EventTarget dispatch lifecycle', () => {
  ...
  describe.each(PRE_DISPATCH_STOP_FORMS)(
    'an event stopped with %s before dispatch',
    (_name, stop) => {
      it.each([false, true])(
        'skips capture=%s listeners and resets propagation state',
        (capture) => {
          const target = new EventTarget();
          const listener = vi.fn();
          const event = new Event('change');

          target.addEventListener('change', listener, {capture});
          stop(event);
