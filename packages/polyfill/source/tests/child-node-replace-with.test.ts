...
describe('ChildNode.replaceWith', () => {
  it('does nothing when the receiver is detached', () => {
    const receiver = document.createElement('div');
    const replacement = document.createElement('span');

    receiver.replaceWith(replacement);

    expect(receiver.parentNode).toBeNull();
    expect(replacement.parentNode).toBeNull();
  });

  it('removes an attached receiver when called without arguments', () => {
    ...
    receiver.replaceWith();

    expect([...parent.childNodes]).toEqual([before, after]);
    expect(before.nextSibling).toBe(after);
    expect(after.previousSibling).toBe(before);
    expect(receiver.parentNode).toBeNull();
  });

  it('replaces an attached receiver with one node', () => {
    const parent = document.createElement('div');
    const before = document.createElement('span');
    const receiver = document.createElement('em');
    ...
    expect([...parent.childNodes]).toEqual([before, replacement, after]);
    expect(before.nextSibling).toBe(replacement);
    expect(replacement.previousSibling).toBe(before);
    expect(replacement.nextSibling).toBe(after);
    expect(after.previousSibling).toBe(replacement);
    expect(receiver.parentNode).toBeNull();
  });

  it('replaces a receiver with its next sibling', () => {
    const parent = document.createElement('div');
    const before = document.createElement('span');
    const receiver = document.createElement('em');
    const next = document.createElement('strong');
    const after = document.createElement('i');
    parent.append(before, receiver, next, after);

    receiver.replaceWith(next);

    expect([...parent.childNodes]).toEqual([before, next, after]);
    expect(before.nextSibling).toBe(next);
    expect(next.previousSibling).toBe(before);
    expect(next.nextSibling).toBe(after);
    expect(receiver.parentNode).toBeNull();
  });
