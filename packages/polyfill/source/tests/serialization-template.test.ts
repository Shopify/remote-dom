...
import {CONTENT, HTML_NAMESPACE, SVG_NAMESPACE} from '../constants.ts';
...
describe('template parsing and serialization', () => {
  ...
  it('does not serialize lazy HTML template ordinary children', () => {
    const container = window.document.createElement('div');
    const template = window.document.createElement(
      'template',
    ) as HTMLTemplateElement;
    const ordinaryChild = window.document.createElement('span');
    ordinaryChild.textContent = 'ordinary';
    template.appendChild(ordinaryChild);
    container.appendChild(template);

    expect(template[CONTENT]).toBeUndefined();
    expect(template.outerHTML).toBe('<template></template>');
    expect(container.innerHTML).toBe('<template></template>');
    expect(template[CONTENT]).toBeUndefined();
    expect([...template.childNodes]).toEqual([ordinaryChild]);
    expect(ordinaryChild.parentNode).toBe(template);
  ...
  it('serializes and clears initialized template content', () => {
    const template = window.document.createElement(
      'template',
    ) as HTMLTemplateElement;
  ...
  it('selects prefixed HTML template content by exact local name', () => {
    const template = window.document.createElementNS(
      HTML_NAMESPACE,
      'h:template',
    ) as HTMLTemplateElement;
    const ordinaryChild = window.document.createElement('span');
    ...
    expect(template.outerHTML).toBe('<h:template></h:template>');

    template.content.append('content');

    expect(template.outerHTML).toBe('<h:template>content</h:template>');
    expect([...template.childNodes]).toEqual([ordinaryChild]);
  });

  it.each([
    ['foreign', 'urn:widget', 'template'],
    ['null', null, 'template'],
    ['established case', HTML_NAMESPACE, 'Template'],
  ])(
    'serializes ordinary children for %s template controls',
    (_label, namespace, name) => {
      const template = window.document.createElementNS(
        namespace as typeof HTML_NAMESPACE,
        name,
      );
      template.append('ordinary');

      expect(template.outerHTML).toBe(`<${name}>ordinary</${name}>`);
    },
  );
