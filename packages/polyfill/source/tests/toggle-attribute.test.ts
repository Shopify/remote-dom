...
describe('Element.toggleAttribute()', () => {
  ...
  it('toggles only the null-namespace attribute', () => {
    const element = document.createElement('div');
    element.setAttributeNS('first', 'state', 'first');
    element.setAttributeNS('second', 'state', 'second');

    expect(element.toggleAttribute('STATE')).toBe(true);
    expect(element.getAttributeNS(null, 'state')).toBe('');
    expect(element.getAttributeNS('first', 'state')).toBe('first');
    expect(element.getAttributeNS('second', 'state')).toBe('second');

    expect(element.toggleAttribute('STATE')).toBe(false);
    expect(element.getAttributeNS(null, 'state')).toBeNull();
    expect(element.getAttributeNS('first', 'state')).toBe('first');
    expect(element.getAttributeNS('second', 'state')).toBe('second');
  });
