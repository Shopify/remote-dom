...
describe('CustomElementRegistry', () => {
  ...
  describe('whenDefined()', () => {
    it.each(['element', 'My-element', 'font-face'])(
      'rejects the invalid name %s with a SyntaxError',
      async (name) => {
        const registry = new CustomElementRegistryImplementation();

        const definition = registry.whenDefined(name);

        await expect(definition).rejects.toBeInstanceOf(DOMException);
        await expect(definition).rejects.toMatchObject({name: 'SyntaxError'});
      },
    );
  });
});
